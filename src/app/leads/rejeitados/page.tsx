...
export default function RejeitadosPage({ searchParams }: PageProps) {
  const { start, end } = searchParams;
  const db = new Database(dbPath, { readonly: true });
  let leads: any[] = [];

  try {
    let whereClause = `funnel_type = 'REJECTED'`;
    if (start && end) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-8 shadow-xl">
        <h2 className="text-xl font-semibold text-white mb-4">Leads Rejeitados</h2>

        {leads.length === 0 ? (
          <p className="text-slate-500 text-center py-8">Nenhum lead rejeitado encontrado.</p>
        ) : (
          <div className="space-y-2">
            {leads.map((lead) => (
              ...
              >
                ...
                <div className="text-slate-400 text-sm italic text-amber-500/80">
                  Rejeitado
                </div>
              </a>
            ))}
