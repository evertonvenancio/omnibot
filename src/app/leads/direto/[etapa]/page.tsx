...
const stageStatusMap: Record<string, string> = {
  ...
  encaminhado: 'forwarded_whatsapp',
};

export default function GenericDrillDownDiretoPage({ params, searchParams }: PageProps) {
  const { etapa } = params;
  ...
  try {
    let whereClause = `funnel_type = 'A_CLIENT' AND pipeline_status = '${targetStatus}'`;
      ...
      <header className="mb-8 flex justify-between items-center border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">
            Clientes diretos / {translatedEtapa}
          </h1>
        </div>
