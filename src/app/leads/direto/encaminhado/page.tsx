...
export default function EncaminhadoDiretoPage({ searchParams }: PageProps) {
  const { start, end } = searchParams;
  const db = new Database(dbPath, { readonly: true });
  let leads: any[] = [];

  try {
    let whereClause = `funnel_type = 'A_CLIENT' AND (pipeline_status = 'forwarded_whatsapp' OR pipeline_status = 'forward_whatsapp')`;
